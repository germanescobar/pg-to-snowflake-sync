...
export async function runFullSync(config: Config): Promise<void> {
  ...
  try {
    ...
    // 2. Get current LSN to start replication after full dump
    const lsnResult = await pgClient.query("SELECT pg_current_wal_lsn() as current_lsn");
    const startLSN = lsnResult.rows[0].current_lsn;
    console.log(`Captured start LSN: ${startLSN}`);
